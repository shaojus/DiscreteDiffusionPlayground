...
class RandomMaskingTransformer(nn.Module):
    def __init__(self, d_model=128, nhead=4, d_hid=512, n_layers=6, dropout=0.2, max_len=16, mask_prob: float=0.3, device=None):
        super().__init__()
        self.MASK = 2
        self.V = 3
    ...
    def forward(self, tok: torch.Tensor):
        emb = self.pos(self.oh[tok])
        out = self.enc(emb)
        return self.fc(out)

    def training_loss(self, batch: torch.Tensor) -> torch.Tensor:
        B, L = batch.shape
        tok = batch.transpose(0, 1).contiguous()
        m = torch.rand_like(tok.float()) < self.mask_prob
        tok_masked = tok.clone()
        tok_masked[m] = self.MASK
        logits = self.forward(tok_masked)
        target = tok[m]
        pred = logits[m, :2]
    ...
    def generate(self, batch: int = 1, temp: float = 1.0, max_steps: int | None = None, record_order: bool = False, parallel: bool = False, top_k: int = 1):
        self.eval()
