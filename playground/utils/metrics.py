...
def get_true_samples(ds, n=100_000, chunk=50_000):
    """
    Draw n samples from the continuous GMM conditioned on being inside [-R,R]^2.
    """
    R = float(ds.R)
    out = []
    need = n
    with torch.no_grad():
        while need > 0:
            xy = ds.dist.sample((chunk,)).cpu().numpy()
            m = (np.abs(xy[:, 0]) <= R) & (np.abs(xy[:, 1]) <= R)
            acc = xy[m]
            if len(acc) == 0:
                continue
            if len(acc) > need:
